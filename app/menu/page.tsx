...
export default async function MenuPage() {
    ...
    return (
        <main className="min-h-screen bg-gradient-to-br from-[#F5E6D3] via-[#F5E6D3] to-[#F4861F]/10">
            ...
            >
                <div
                    className="absolute inset-0 bg-gradient-to-b from-[#C84C28]/95 to-[#8B2500]/95"
                    aria-hidden="true"
                />
            ...
            >
                {/* Navigation Menu */}
                <nav
                    className="flex flex-wrap justify-center mb-12 sm:mb-16 md:mb-20 gap-4 sm:gap-6"
                    aria-label="Filtros do menu"
                >
                    {['Menu Principal', 'Bebidas', 'Especiais'].map((item) => (
                        <button
                            key={item}
                            className="px-6 sm:px-8 py-2.5 sm:py-3 rounded-2xl bg-white/50 backdrop-blur-sm text-[#2B4C5C] hover:text-[#C84C28] hover:bg-[#FFB800]/20 transition-all duration-200 text-base sm:text-lg font-medium shadow-md hover:shadow-lg"
                ...
                {/* Menu Items Grid */}
                {[...almoco, ...jantar, ...sobremesas].length > 0 ? (
                    <div
