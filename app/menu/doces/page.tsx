...
export default async function Doces() {
    const items = await db.query.menuItems.findMany({
        where: and(
            eq(menuItems.isDoce, true),
    ...
    return (
        <main className="min-h-screen bg-gradient-to-br from-[#F5E6D3] via-[#F5E6D3] to-[#F4861F]/10">
            {/* Hero Banner */}
            <section className="relative h-[50vh] min-h-[400px] bg-[url('/doces-banner.jpg')] bg-cover bg-center">
                ...
                <header className="absolute inset-0 flex flex-col items-center justify-center text-center p-4 z-10">
                    <div className="bg-[#F5E6D3]/10 p-12 rounded-3xl backdrop-blur-md border-2 border-[#FFB800] max-w-3xl mx-auto shadow-2xl">
                        <h1 className="text-7xl font-bold text-white font-serif mb-6 drop-shadow-lg">Doces</h1>
                        ...
                        <p className="text-white italic text-2xl font-medium tracking-wide drop-shadow-lg">Delícias & Tentações</p>
                    </div>
                </header>
